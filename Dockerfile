...
ADD ./liferay-portal-tomcat-6.2.5.4.zip /
# Unzip the Portal Tomcat Bundle
RUN unzip -qq liferay-portal-tomcat-6.2.5.4.zip

# Default customized portal-ext.properties that will enable the users
# to point the Liferay home to the mount volume
ADD ./portal-ext.properties /liferay-portal-6.2.5.4/tomcat-7.0.62/webapps/ROOT/WEB-INF/classes/portal-ext.properties

# Remove the bundle
RUN rm liferay-portal-tomcat-6.2.5.4.zip

# Setup Mount Volume Directories
RUN mkdir -p /liferay/deploy && mv liferay-portal-6.2.5.4/data /liferay/data
...
WORKDIR /liferay-portal-6.2.5.4
...
ONBUILD ADD ./lib    /liferay-portal-tomcat-6.2.5.4/tomcat-7.0.62/lib
ONBUILD COPY ./bin/*.sh /liferay-portal-tomcat-6.2.5.4/tomcat-7.0.62/bin/
